...
#include "/Users/tobyzhao/Desktop/TECHIN515_Sp25-main/lab3/src/sorting_hat_model.h"  // ML Model
        ...
        if ((millis() - lastDebounceTimes[i]) > debounceDelay) {
            if (reading != buttonStates[i]) { // If the button state has changed
                ...
                if (buttonStates[i] == LOW) { // If the button is pressed (LOW because of INPUT_PULLUP)
                    responses[questionIndex] = buttonLabel[i]; // Save the user's answer (1–4)
                    buttonPressed = true; // Note that a button was pressed
                }
            }
        }
